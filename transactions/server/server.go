...
func (s *Server) Run() {
	go func() {
		...
		pb.RegisterTransactionsAPIHandlerServer(context.Background(), mux, s)

		log.Fatalln(http.ListenAndServe("0.0.0.0:8005", mux))
	}()
	...
	pb.RegisterTransactionsAPIServer(grpcServer, s) // use authogenerated code to register the server
	reflection.Register(grpcServer)

	log.Printf("Starting Transactions server on port %s", port)
	go func() {
